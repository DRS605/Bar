...
public sealed class RegistrarUsuario
{
    ...
    public async Task<Resultado<PerfilUsuario>> EjecutarAsync(RegistrarUsuarioComando comando, CancellationToken ct = default)
    {
        ...
        if (usuario.EsFallo)
        {
            return Resultado.Fallo<PerfilUsuario>(usuario.Error);
        }

        _usuarios.Agregar(usuario.Valor);
        await _unidadDeTrabajo.GuardarCambiosAsync(ct).ConfigureAwait(false);

        await _verificacionEmail.EnviarVerificacionAsync(usuario.Valor, ct).ConfigureAwait(false);

        return Resultado.Ok(PerfilUsuario.Desde(usuario.Valor));
    }
